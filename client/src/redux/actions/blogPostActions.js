...
import { setBlogPostByCategory, setLoading, setError, blogPostCreated, blogPostRemoved, blogPostUpdated } from '../slices/blogPost';
const host = 'http://localhost:10000'
export const getBlogPostsByCategory = (category, pageItems) => async (dispatch) => {
    dispatch(setLoading(true));

    try {
        const {data} = await axios.get(`${host}/api/blog-posts/`);
