...
import { Input, Textarea, Button, VStack, FormControl, FormLabel, Container, FormErrorMessage, Heading, useToast, Text} from '@chakra-ui/react';
...
const ContactScreen = () => {
    ...
    const sendEmail = (values, reset)=>
        emailjs.send(emailjsInfo.service_id, emailjsInfo.template_id, values,emailjsInfo.public_key).then((res)=>{
            toast({title: 'Email send.', description: 'Thanks! I will come back to you soon.', status: 'success', duration: 7000, isClosable: true,});
            reset();
            setTimeout(() => {
                navigate('/', {replace: true})
            }, 1000);
        }).catch((error)=>{
            toast({title: 'Whoops!', description: 'Something went wrong. Please try again later!', status: 'error', duration: 7000, isClosable: true,});
                ...
                <form onSubmit={handleSubmit}>
                    <VStack spacing='5'>
                        ...
                        <FormControl isInvalid={!!errors.text && touched.text}>
                            ...
                            <Field as={Input} id='text' name='text' rows='5' type='textarea' variant='filled' 
                            validate={(value)=>{
                                let error;
                                if(value.length < 10){
